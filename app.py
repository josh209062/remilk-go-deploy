...
st.set_page_config(
    page_title="Milk Detection Using Streamlit",
    page_icon="🥛",
    layout="wide"
)

# cfg_model_path = 'models/yolov5s.pt'
cfg_model_path = 'models/best-MN2.pt'
model = None
confidence = .25
...
def load_model(path, device):
    model_ = torch.hub.load('ultralytics/yolov5',
...
def main():
    ...
    if not os.path.isfile(cfg_model_path):
        ...
        confidence = st.sidebar.slider(
            '👁‍🗨 Confidence Level', min_value=0.1, max_value=0.8, value=.40)
        ...
        if input_option == '📷 Detect Via Image':
            data_src = st.sidebar.radio("Select input source: ", [
                'Sample data', 'Upload your own data'], index=1)
            ...
            with col2:
                ...
                st.write(" ")
                st.write(" ")
                st.image("https://telegra.ph/file/ecce29e0e57aebb4a2c20.png")
            with col4:
                st.write(" ")
                st.write(" ")
                st.write(" ")
                st.write(" ")
                st.write(" ")
                st.image("https://telegra.ph/file/1d478d3c83fa221ffdbd4.png")
            with col5:
                pass
            with col3:
                st.image(
